...
var TextureLoader = (function(){
	...
	var _preLoad = function( scene ){
		
		//if( !_assetsManager ){
		//	_assetsManager = new BABYLON.AssetsManager( scene );
		//}

		var obj = _textureQueue.pop();
		if(obj){

			//var textureTask = assetsManager.addTextureTask(obj.name, obj.src );
			//textureTask.onSuccess = function(task) {
			//	_textureRegistry[obj.name] = task.texture;
			//	_preLoad();
			//}
			...
			imageObj.onload = function() {	
				...
				_textureRegistry[obj.name] = obj.src;
...
var WorldSynthesizer = (function(){
	...
	var SCENE_BG  =  new BABYLON.Color4(0,0,0,0.000000001);
	...
	var _generateMaterial = function( scene ){
		...
		m2.diffuseTexture = new BABYLON.Texture(TextureLoader.load('building'), scene);
	...
	var _loadWorld = function(){
		...
		TextureLoader.setCallback( WorldSynthesizer.buildScene );
		TextureLoader.preLoad( _scene );
	...
	var _init = function(){
		...
		TextureLoader.addTexture( 'building', 'textures/building.jpg');
		for( var i = 100; i <= 109; i++){ TextureLoader.addTexture( 'r' + i , 'textures/roads/' + i + '.png'); }
		_loadWorld();
	};
	
	return { init: _init, buildScene: _buildScene, end: _endAnimation };
})();